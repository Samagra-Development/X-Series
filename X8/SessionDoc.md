...
4. Airflow is a WMS **that defines tasks and and their dependencies as code**, executes those tasks on a regular schedule, and distributes task execution across worker processes.
...
> Requirement: Docker
1. In order to use Airflow in click to run state we have dockerised a version of our airflow in our own template format. You can get it from [here](https://github.com/Samagra-Development/airflow)
2. Set up any  is **secrets management tool specifically designed to control access to sensitive credentials in a low-trust environment** if you have any in the .env file in the project root directory.
...
4. Create a folder named dags inside project directory. *This is the default path to store your dags*
    ...
    └── script
        └── entrypoint.sh	
    ```
5. You can update the airflow config from the airflow.cfg under config folder 
6. Steps to create new users and and dependecies can be found under **README** file in the samagra airflow [repository](https://github.com/Samagra-Development/airflow)
7. You can check the aiflow ui running on http://localhost:8181
